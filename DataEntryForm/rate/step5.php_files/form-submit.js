...
DHTMLSuite.form.prototype = 
{
	...
	submit : function()
	{
		//alert( this.formRef + "---" + this.formRef.action ) ;
		if( document.getElementById("messenger-p") )
		{
			var objBody = document.getElementById("messenger-p") ;

			var processstep = document.createElement("div");

			if(processstep)
			{
				/*if(navigator.userAgent.indexOf("Chrome") != -1 && cntUnique <= 1)
				{	
					//alert("here go") ;
					cntUnique++ ;
				}
				else if(navigator.userAgent.indexOf("Safari") != -1 && cntUnique <= 1)
				{
					cntUnique++ ;
				}
				else if(navigator.userAgent.indexOf("Opera") != -1 && cntUnique <= 1)
				{
					cntUnique++ ;
				}*/
				if(navigator.userAgent.indexOf("MSIE") != -1 && cntUnique <= 1)
				{	
					processstep.style.marginTop = -30+"px" ;
					processstep.style.marginLeft = -300+"px" ;
				}
				processstep.setAttribute('id','showProcessingMsg_p');
				processstep.style.position = 'absolute' ;
				processstep.style.zIndex = 10000 ;
				processstep.innerHTML = "<img src='/images/processing.gif'> Processing... Please wait..." ;
				processstep.className = "processing" ;
				objBody.appendChild(processstep);
			}
		}
	...
	__getIframeResponse : function()
	{
		if(this.responseEl){		
			if(this.responseFile){
				...
				if( document.getElementById( "showProcessingMsg_p" ) )
				{
					var olddiv = document.getElementById("showProcessingMsg_p");
				...
				{
					if( preEvent && editBasicElem == "" ) {
						loadEventModules( preEvent ) ;
						cntUnique = 1 ;
					} else {
						ajaxResponseScript( self.frames[this.iframeName].document.body.innerHTML ) ;
						cntUnique = 1 ;
					}
				}
			}						
